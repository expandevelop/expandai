...
export default function OperatorsPage() {
  ...
  return (
    <AppShell
      ...
      description="Esta rota dedicada concentra a leitura das operadoras já persistidas no backend, oferecendo contexto de status, datas e posição operacional dentro da ExpandAI."
    >
      {error ? (
          ...
          <section className="mt-6 rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <SectionHeader
                ...
                description="A tabela abaixo prepara o frontend para evoluir futuramente em direção a filtros, detalhes por entidade e ações autenticadas de manutenção cadastral."
              />
              <button
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {data.operators.map((operator) => (
                      <tr key={operator.id} className="align-top">
                        <td className="px-4 py-4 font-medium text-white">{operator.tradeName}</td>
                        <td className="px-4 py-4 text-slate-300">{operator.legalName}</td>
                        <td className="px-4 py-4">
                          <StatusBadge value={operator.status} />
                        </td>
                        <td className="px-4 py-4 text-slate-400">
                          {formatCompactDate(operator.createdAt)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
